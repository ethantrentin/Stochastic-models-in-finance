...
strike_min = st.number_input("Strike minimum", value=80, step = 1)
strike_max = st.number_input("Strike maximum", value=120, step = 1)
if strike_min > strike_max:
    st.error("Please enter a valid range : Strike minimum should be less than Strike maximum")
    st.stop()
else:
    strike_linspace = np.linspace(strike_min, strike_max, 100)
